...
export default function CircuitKolnaNemchiw({navigation, route}) {
  ...
  //     const response = await axios.get(`${baseURL}/mounements/kolna_nemchiw_LatLing`);
    ...
    if (id) {
      if (pathSite > 30) {
        axios({
          ...
          data: {
            text_notification:
              "Vous êtes hors périmètre! Envie de commencer l'expérience?  Veuillez rester à proximité du site archéologique (Max 30km)",
            UtilisateurId: id,
          },
        })
          .then(response => {
  ...
  const fetchUserLocation = async () => {
    await Geolocation.watchPosition(
      ...
      //  console.log('distanceCercle', intersection_30km_Route);
       // console.log('frommm', from);
        ...
        for (let i = 0; i < intersection_30km_Route.length; i++) {
          ...
          if (distanceCercle < minDist) {
           ...
           // setNearestCircle( intersection_30km_Route[i] );
    ...
    <View style={styles.page}>
      <View style={styles.container}>
       ...
       <Loader/>
          ...
          (!toCarthage ? (
            <MapboxGL.MapView
              ...
              <MapboxGL.Camera
                ...
                centerCoordinate={[10.331699, 36.851647]}
              />
              ...
              <PathDrawer
                coords={circuit_Kolna_Nemchiw}
                lineColor="#e59138"
                lineDasharray={[3, 3]}></PathDrawer>

              {pathSite <= 30 && mapLoadded && (
                <View>
                  <MapboxGL.ShapeSource
        ...
        {pathSite > 30 && !toCarthage && mapLoadded && (
          <View
            style={{
              width: '90%',
              position: 'absolute',
